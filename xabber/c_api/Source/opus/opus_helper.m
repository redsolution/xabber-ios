...
- (void) setBitrate:(NSUInteger)bitrate {
    ...
    dispatch_async(self.processingQueue, ^{
        opus_encoder_ctl(self->_encoder, OPUS_SET_BITRATE(bitrate));
    });
}
...
- (BOOL) createEncoder: (int) sampleRate {
    ...
    self.encoder = opus_encoder_create(sampleRate, 1, OPUS_APPLICATION_AUDIO, &opusError);
...
- (NSData*) encode:(NSData*) pcmData frameSize:(int) frameSize{
    ...
    if (encodedByteCount < 0) {
        ...
        return nil;
    } else if (encodedByteCount == 0){
        NSLog(@"zero bytes encoded");
    }
                                     ...
                                     float manual_gain, int *streams, int wav_format)
{
    ...
    *channels = header.channels;
    ...
    if(header.input_sample_rate>0)*rate=header.input_sample_rate;
                              ...
                              int *skip, int file, opus_int64 maxout, int fp)
{
    ...
    for (i=0;i<(int)out_len*channels;i++){
        out[i]=float2int(fmaxf(-32768,fminf(output[i]*32768.f,32767)));
    }
