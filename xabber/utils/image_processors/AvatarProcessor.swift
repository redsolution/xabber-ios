...
struct AvatarProcessor: ImageProcessor {
    ...
    public init() {
        self.targetContentMode = .aspectFill
        self.minimumSize = CGSize(square: 64)
        self.anchor = CGPoint(x: 0.5, y: 0.5)
        self.identifier = "xabber.com.avatar"
    }
    ...
    public func process(item: ImageProcessItem, options: KingfisherParsedOptionsInfo) -> KFCrossPlatformImage? {
        ...
        case .image(let image):
            ...
            let roundingCorners: RectCorner = .all
            let cornerRadius: CGFloat = referenceSize.height / 2
            if image.size == referenceSize {
                return image.kf.image(withRoundRadius: cornerRadius,
                                      fit: referenceSize,
