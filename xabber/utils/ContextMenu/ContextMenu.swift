...
public class ContextMenuConstants {
    ...
    public var BlurEffectDefault : UIBlurEffect = UIBlurEffect(style: .regular)
...
open class ContextMenu: NSObject {
    ...
    // MARK:- open Variables
    open var currentMessagePrimary: String?
    ...
    open var onItemTap : ((_ value: String) -> Bool)?
    ...
    init?(viewTargeted: UIView, window: UIView? = nil) {
        if let wind = window ?? UIApplication.shared.windows.first ?? UIApplication.shared.windows.first(where: {$0.isKeyWindow}) {
            self.customView = wind
    ...
    public func updateWindow(window: UIView? = nil) {
//        let wind = window ?? UIApplication.shared.windows.first ?? UIApplication.shared.windows.first(where: {$0.isKeyWindow})
//        self.customView = wind!
//        if let frame = window?.bounds {
//            self.mainViewRect = frame
//        }
    }
    ...
    open func showMenu(viewTargeted: UIView, delegate: ContextMenuDelegate, animated: Bool = true){
        ...
        let allItems: [ContextMenuItem] = self.items.flatMap({ $0 })
        let sectionsHeight = CGFloat(self.items.count - 1) * CGFloat(8.0)
        ...
        DispatchQueue.main.async {
            ...
            if !self.items.isEmpty {
                self.menuHeight = sectionsHeight + (CGFloat(allItems.count) * self.MenuConstants.ItemDefaultHeight) + (self.headerView?.frame.height ?? 0) + (self.footerView?.frame.height ?? 0) // + CGFloat(self.items.count - 1)
            } else {
    ...
    func updateHorizontalTargetedImageViewRect() {
        ...
        if leftClippedSpace > 0, rightClippedSpace > 0 {
            ...
            mX = tvX + mW - MenuConstants.HorizontalMarginSpace
...
//            return nil
//        } else {
//            return UIView(frame: CGRect(origin: .zero, size: CGSize(width: 8, height: 8)))
//        }
//    }
