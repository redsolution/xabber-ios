...
struct ToastPresenter {
    ...
    func present(message: String, image: UIImage? = nil, danger: Bool = false) {
        (UIApplication.shared.keyWindow?.rootViewController?.presentedViewController ?? UIApplication.shared.keyWindow?.rootViewController)?.view.makeToast(
            message,
            duration: 2.0,
            title: nil,
            image: image,
            danger: danger) { tap in
                if tap {
                    (UIApplication.shared.keyWindow?.rootViewController?.presentedViewController ?? UIApplication.shared.keyWindow?.rootViewController)?.view.hideToast()
                }
            }
    }
