...
//MARK: - Protocols
protocol TappedPhotoInMediaGallery {
    func tappedPhoto(primary: String)
}

protocol CellPhotoIsMissing {
    func passCellToFooter(cell: UICollectionViewCell)
}


//MARK: - Extensions
extension InfoScreenFooterView: TappedPhotoInMediaGallery {
    func tappedPhoto(primary: String) {
        chatsDelegate?.didTapPhotoFromGallery(primary: primary)
    }
}

extension InfoScreenFooterView: CellPhotoIsMissing {
    func passCellToFooter(cell: UICollectionViewCell) {
        guard let index = mediaCollectionView.indexPath(for: cell) else { return }
