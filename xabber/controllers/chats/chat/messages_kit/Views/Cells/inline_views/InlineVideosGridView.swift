        ...
        prepareGrid(references).enumerated().forEach {
            ...
            if index < urls.count {
                ...
                if references[index].isDownloaded {
                    self.videoPlayIcon.image = #imageLiteral(resourceName: "play").withRenderingMode(.alwaysTemplate)
                } else {
                    self.videoPlayIcon.image = #imageLiteral(resourceName: "download").withRenderingMode(.alwaysTemplate)
                }
                ImageCache.default.retrieveImage(forKey: key) { result in
                    ...
                    case .failure(let value):
                        self.errorImageView.image = #imageLiteral(resourceName: "video-off-outline").withRenderingMode(.alwaysTemplate)
