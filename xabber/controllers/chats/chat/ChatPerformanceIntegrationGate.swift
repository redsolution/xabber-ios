...
/// Process-local evidence emitted only after the production initial-frame
/// datasource transaction has committed its viewport. Unlike the fixture
/// launch plan, every field below is captured from the accepted descriptor,
/// prepared Realm frame, background mapper and committed UIKit transaction.
/// The accessibility receipt projects only numeric values and closed enums;
/// the viewport anchor itself never crosses that boundary.
struct ChatPerformanceInitialFrameCommitDiagnostics: Equatable {
    /// Exact owner of the committed initial-frame publication. Semantic
    /// descriptor equality is insufficient because a replacement may remap
    /// the same request under a newer presentation generation while an older
    /// hosted proof is still queued.
...
enum ChatPerformanceFixtureRemoteHistoryActionKind: Equatable {
    case anchorContextPrefetch
}

enum ChatPerformanceFixtureArchiveTransportKind: Equatable {
    case initialBootstrap
    case detachedPage
}

enum ChatPerformanceFixtureArchiveLeasePurpose: Equatable {
    case initialBootstrap
...
enum ChatPerformanceFixtureArchiveSemanticRouteClass: Equatable {
    case latest
    case unreadBoundary
    case savedPosition
    case exactTarget
...
/// Process-local mirror of the canonical MAM plan accepted by production.
/// Raw cursors and target IDs deliberately stay inside the app process; only
/// the closed cursor kind and numeric counters may reach accessibility.
struct ChatPerformanceFixtureArchiveRequestDescriptor: Equatable {
    let requestKind: MessageArchiveManager.RegularChatArchiveRequestKind
    let archivePurpose: MessageArchiveManager.RequestPurpose
    let leasePurpose: ChatPerformanceFixtureArchiveLeasePurpose
    let direction: ChatHistoryPageDirection?
    let cursorKind: ChatOpenRealPipelineFixtureArchiveCursorKind
    let cursorArchiveID: String?
    let targetArchiveID: String?
    ...
    static func make(
        plan: MessageArchiveManager.RegularChatArchiveRequestPlan,
        leasePurpose: ChatPerformanceFixtureArchiveLeasePurpose,
        requestSource: ChatOpenMessageRequestSource?,
        semanticRouteClass:
            ChatPerformanceFixtureArchiveSemanticRouteClass
    ) -> Self? {
        guard plan.conversationType == .regular,
              plan.max > 0,
              plan.max <= ChatHistoryPagingConfiguration.pageSize else {
            return nil
        }
        ...
        switch plan.kind {
        case .bootstrap:
            switch plan.coverageUpdateKind {
            case .bootstrapNewest:
                direction = nil
                cursorKind = .latest
                cursorArchiveID = nil
            case .pageOlder(let cursor):
                direction = .older
                cursorKind = .before
                cursorArchiveID = cursor
            case .pageNewer(let cursor):
                direction = .newer
                cursorKind = .after
                cursorArchiveID = cursor
            case .disjointWindow:
                direction = nil
                cursorKind = .aroundTarget
                cursorArchiveID = nil
            case .gapRepairOlder(let cursor):
                direction = .older
                cursorKind = .before
                cursorArchiveID = cursor
            case .gapRepairNewer(let cursor):
                direction = .newer
                cursorKind = .after
                cursorArchiveID = cursor
            case .none:
            ...
            targetArchiveID = plan.ids?.count == 1 ? plan.ids?.first : nil
        case .older:
            direction = .older
            cursorKind = .before
            cursorArchiveID = plan.coverageUpdateKind.adjacencyCursorArchiveId
            targetArchiveID = nil
        case .newer:
            direction = .newer
            cursorKind = .after
        ...
        case .exactAnchor:
            ...
            cursorKind = .aroundTarget
            cursorArchiveID = nil
            targetArchiveID = plan.ids?.first
        case .dateWindow:
            guard plan.start != nil,
                  plan.end != nil else {
                return nil
            }
            direction = nil
        ...
        let descriptor = Self(
            ...
            archivePurpose: plan.purpose,
            leasePurpose: leasePurpose,
            direction: direction,
            cursorKind: cursorKind,
            cursorArchiveID: cursorArchiveID,
            targetArchiveID: targetArchiveID,
            maximumResultCount: plan.max,
            requestSource: requestSource,
            semanticRouteClass: semanticRouteClass
        )
        return descriptor.hasConsistentSemanticShape ? descriptor : nil
    }

    private var hasConsistentSemanticShape: Bool {
        switch semanticRouteClass {
        case .latest:
            return leasePurpose == .initialBootstrap &&
                requestKind == .bootstrap &&
                archivePurpose == .bootstrap &&
                cursorKind == .latest &&
                direction == nil &&
                requestSource == nil
        case .unreadBoundary:
        ...
        case .exactTarget:
            return leasePurpose == .anchorTransaction &&
                (requestKind == .exactAnchor || requestKind == .dateWindow) &&
                archivePurpose == .jump &&
                cursorKind == .aroundTarget &&
                requestSource != nil
        case .anchorContext:
            return leasePurpose == .anchorTransaction &&
                (requestKind == .older || requestKind == .newer ||
                    requestKind == .gapRepair) &&
                (archivePurpose == .pageOlder ||
                    archivePurpose == .pageNewer ||
                    archivePurpose == .gapRepair) &&
                direction != nil &&
                requestSource != nil
        case .knownGapRepair:
            return leasePurpose == .interactivePaging &&
                requestKind == .gapRepair &&
                archivePurpose == .gapRepair &&
                direction != nil
        case .interactivePage:
            return leasePurpose == .interactivePaging &&
                ...
                direction != nil
        case .backgroundContext:
            return leasePurpose == .detachedBackground &&
                (requestKind == .older || requestKind == .newer ||
                    requestKind == .gapRepair) &&
                direction != nil
        }
...
enum ChatPerformanceFixtureArchivePayloadAdmissionPolicy {
    static func accepts(
        ...
        guard descriptor.requestSource == expectedSource,
              descriptor.semanticRouteClass == expectedSemanticRouteClass,
              deliveredArchiveIDs.count <= descriptor.maximumResultCount,
              firstArchiveID == deliveredArchiveIDs.first,
              lastArchiveID == deliveredArchiveIDs.last else {
            return false
        }
        if descriptor.requestKind == .exactAnchor {
            return deliveredArchiveIDs.count == 1 &&
                deliveredArchiveIDs.first == descriptor.targetArchiveID
        }
        if descriptor.semanticRouteClass == .anchorContext {
            guard let expectedDeliveredArchiveIDs else { return false }
            return deliveredArchiveIDs == expectedDeliveredArchiveIDs
        }
        if let expectedDeliveredArchiveIDs {
            return deliveredArchiveIDs == expectedDeliveredArchiveIDs
        }
        return true
    }
...
enum ChatPerformanceFixtureInteractiveGapDescriptorAdmissionPolicy {
    static func accepts(
        descriptor: ChatPerformanceFixtureArchiveRequestDescriptor,
        expectedDirection: ChatHistoryPageDirection,
        expectedCursorArchiveID: String?,
        expectedPageSize: Int
    ) -> Bool {
        guard let expectedCursorArchiveID,
              !expectedCursorArchiveID.isEmpty else {
            return false
        }
        let expectedCursorKind: ChatOpenRealPipelineFixtureArchiveCursorKind =
            expectedDirection == .older ? .before : .after
        return descriptor.semanticRouteClass == .knownGapRepair &&
            descriptor.leasePurpose == .interactivePaging &&
            descriptor.requestKind == .gapRepair &&
            descriptor.archivePurpose == .gapRepair &&
            descriptor.direction == expectedDirection &&
            descriptor.cursorKind == expectedCursorKind &&
            descriptor.cursorArchiveID == expectedCursorArchiveID &&
            descriptor.maximumResultCount == expectedPageSize &&
            descriptor.requestSource == nil
    }
}

struct ChatPerformanceFixtureTargetWindowContextShape: Equatable {
    let olderCount: Int
    let newerCount: Int
}

enum ChatPerformanceFixtureTargetWindowDescriptorAdmissionPolicy {
    static func contextShape(
        targetOrdinal: Int,
        totalMessageCount: Int,
        pageSize: Int = ChatInitialFirstFrameHistoryConfiguration.pageSize
    ) -> ChatPerformanceFixtureTargetWindowContextShape? {
        guard totalMessageCount > 0,
              targetOrdinal >= 0,
              targetOrdinal < totalMessageCount,
              pageSize > 0 else {
            return nil
        }
        let contextBudget = min(pageSize - 1, totalMessageCount - 1)
        var olderCount = min((contextBudget + 1) / 2, targetOrdinal)
        var newerCount = min(
            contextBudget / 2,
            totalMessageCount - targetOrdinal - 1
        )
        var remaining = contextBudget - olderCount - newerCount
        if remaining > 0 {
            let newerCapacity = totalMessageCount - targetOrdinal - 1 - newerCount
            let newerTopUp = min(remaining, max(0, newerCapacity))
            newerCount += newerTopUp
            remaining -= newerTopUp
        }
        if remaining > 0 {
            let olderCapacity = targetOrdinal - olderCount
            olderCount += min(remaining, max(0, olderCapacity))
        }
        return ChatPerformanceFixtureTargetWindowContextShape(
    ...
    static func accepts(
        ...
        guard descriptor.semanticRouteClass == .anchorContext,
              descriptor.leasePurpose == .anchorTransaction,
              descriptor.cursorArchiveID == targetArchiveID,
        ...
        case .older:
            return descriptor.requestKind == .older &&
                descriptor.archivePurpose == .pageOlder &&
                descriptor.cursorKind == .before &&
                descriptor.maximumResultCount == shape.olderCount
        case .newer:
            return descriptor.requestKind == .newer &&
                descriptor.archivePurpose == .pageNewer &&
                descriptor.cursorKind == .after &&
                descriptor.maximumResultCount == shape.newerCount
        case nil:
            return false
        }
    }
...
struct ChatPerformanceFixtureArchiveTransportRequest: Equatable {
    let kind: ChatPerformanceFixtureArchiveTransportKind
    let queryIds: Set<String>
    let descriptorsByQueryId:
        [String: ChatPerformanceFixtureArchiveRequestDescriptor]
}

struct ChatPerformanceFixtureArchiveTransportSession {
    let stream: XMPPStream
    let archiveManager: MessageArchiveManager
    let messageManager: MessageManager
}

struct ChatPerformanceFixtureRemoteHistoryAction: Equatable {
    let kind: ChatPerformanceFixtureRemoteHistoryActionKind
    let source: ChatOpenMessageRequestSource
    let newerPageSize: Int?
    let olderPageSize: Int?

    var requiresRemoteFetch: Bool {
        (newerPageSize ?? 0) > 0 || (olderPageSize ?? 0) > 0
    }

    var requestedMessageCount: Int {
        max(0, newerPageSize ?? 0) + max(0, olderPageSize ?? 0)
    }
}

enum ChatPerformanceFixtureRemoteHistoryDisposition: Equatable {
    case useProductionTransport
    case consumedByFixtureTransport
}

enum ChatPerformanceFixtureRemoteHistoryRoutingPolicy {
    static func shouldDispatchProductionTransport(
        requiresRemoteFetch: Bool,
        fixtureDisposition: ChatPerformanceFixtureRemoteHistoryDisposition?
    ) -> Bool {
        guard requiresRemoteFetch else { return false }
        return fixtureDisposition != .consumedByFixtureTransport
    }
}
...
struct ChatOpenRealPipelineFixturePlan: Equatable {
    ...
    var interactiveGapInjectionOrdinalRange: Range<Int>? {
        switch scenario {
        case .olderCrossingGap:
            return 160..<240
        case .newerCrossingGap:
            return 80..<160
        default:
            return nil
        }
    ...
    var expectsLinkedPagingTrace: Bool {
        scenario == .olderCrossingGap || scenario == .newerCrossingGap
    }

    var expectedInteractivePagingDirection: ChatHistoryPageDirection? {
        switch scenario {
        case .olderCrossingGap:
            return .older
        case .newerCrossingGap:
    ...
    var expectedInteractivePagingCursorOrdinal: Int? {
        switch scenario {
        case .olderCrossingGap:
            return 240
        case .newerCrossingGap:
