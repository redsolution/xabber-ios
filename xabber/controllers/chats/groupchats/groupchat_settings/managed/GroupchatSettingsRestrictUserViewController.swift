...
//import RxRelay
//
//class GroupchatSettingsRestrictUserViewController: SimpleBaseViewController {
//    
...
//                let out = Datasource(kind: .permission, title: item.displayName, value: item.name, status: item.status, defaultStatus: defaultPermissionStatus, key: item.name)
//                if let expires = item.expires, expires > 0 {
//                    let period: Double = expires - Date().timeIntervalSince1970
//                    let day: Double = period / Double(24 * 60 * 60)
//                    let hour: Double = (remainder(period, 24 * 60 * 60) / (60 * 60)).rounded()
//                    let minute: Double = 60 - abs((remainder(period, 24 * 60 ) / ( 60)).rounded())
//                    out.customiPeriodDay = Int(day)
//                    out.customiPeriodHour = Int(hour)
//                    out.customiPeriodMins = Int(minute)
//                }
//                return out
//            },
//            [
...
//}
//
//extension GroupchatSettingsRestrictUserViewController: UITableViewDelegate {
...
//                let picker = DateTimePickerPresenter()
...
//}
//
//extension GroupchatSettingsRestrictUserViewController: UITableViewDataSource {
...
//            (self.tableView.cellForRow(at: IndexPath(row: index, section: 0)) as? SettingsSwitchCell)?.updateTimer(isForever: item.selectedForever, day: item.customiPeriodDay, hour: item.customiPeriodHour, mins: item.customiPeriodMins)
//        } else {
...
//            (self.tableView.cellForRow(at: IndexPath(row: index, section: 0)) as? SettingsSwitchCell)?.updateTimer(isForever: false, day: nil, hour: nil, mins: nil)
//        }
...
//                }
//                
//                cell.configure(title: item.title, isOn: item.status, key: item.value, isChanged: item.changed, originalStatus: item.defaultStatus, isForever: item.selectedForever, day: item.customiPeriodDay, hour: item.customiPeriodHour, mins: item.customiPeriodMins)
...
//}
//
//extension GroupchatSettingsRestrictUserViewController: TimePickerAlertControllerDelegate {
//    
