...
protocol GroupchatEditContactDelegate {
    func onChangeState(_ itemId: String, sender: UISwitch?)
    func onUpdateTextField(_ itemId: String, value: String?)
}

extension GroupchatEditContactViewController: GroupchatEditContactDelegate {
    
    func onChangeState(_ itemId: String, sender: UISwitch?) {
        func updateDatasource(value: String?) {
            DispatchQueue.main.async {
                if let section = self.datasource.firstIndex(where: { $0.rows.contains(where: { $0.itemId == itemId }) }) {
                    print(section)
                    if let row = self.datasource[section].rows.firstIndex(where: { $0.itemId == itemId }) {
                        print(row)
                        self.datasource[section].rows[row].value = value
...
//                        .sorted(by: { $0[0].value < $1[0].value })
    ...
    func onUpdateTextField(_ itemId: String, value: String?) {
        switch itemId {
        case "nickname": modifiedNickname.accept(value)
        case "badge": modifiedBadge.accept(value)
        default: break
        }
    }
