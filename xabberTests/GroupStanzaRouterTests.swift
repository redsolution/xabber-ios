...
final class GroupStanzaRouterTests: XCTestCase {
    ...
    func testUncorrelatedIQAndLegacyIQPayloadAreRejected() throws {
        ...
        XCTAssertNil(try GroupStanzaRouter.route(legacy, correlating: ["legacy"]))
    }

    func testSubscribedPresenceProducesActivationThenAuthoritativeSnapshot() throws {
        ...
        XCTAssertEqual(input.events[0], .selfSubscription(.both))
        guard case let .snapshot(snapshot) = input.events[1] else {
