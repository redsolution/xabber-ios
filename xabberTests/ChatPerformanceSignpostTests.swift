...
final class ChatPerformanceSignpostTests: XCTestCase {
    ...
    func testSkeletonCommitReceiptCannotFollowLeaseRegistrationInSyncOrAsyncProductionPaths() throws {
        let subscriptionSource = try productionSource(
            "xabber/controllers/chats/chat/rx/ChatViewController+HighPrioritySubscribtions.swift"
        )
        let subscribe = try sourceSlice(
            subscriptionSource,
            from: "internal func subscribe() throws",
            to: "internal func requestInitialBootstrapArchive("
        )
        let skeletonApply = try XCTUnwrap(
            subscribe.range(of: "self.applyBootstrapViewState(")
        )
        let synchronousCommit = try XCTUnwrap(
            subscribe.range(of: "synchronousSkeletonCommit: initialBootstrapViewState == .skeleton")
        )
        let archiveRequest = try XCTUnwrap(
            subscribe.range(of: "self.requestInitialBootstrapArchive()")
        )
        XCTAssertLessThan(skeletonApply.lowerBound, synchronousCommit.lowerBound)
        XCTAssertLessThan(synchronousCommit.lowerBound, archiveRequest.lowerBound)

        let request = try sourceSlice(
            subscriptionSource,
            from: "internal func requestInitialBootstrapArchive(",
            to: "internal func handleSyncChatStartResult("
        )
        let receiptGate = try XCTUnwrap(
            request.range(of: ".hasRecordedPresentationReceipt(")
        )
        let leaseAcquire = try XCTUnwrap(
            request.range(of: "coordinator.acquireOrJoin(")
        )
        XCTAssertLessThan(receiptGate.lowerBound, leaseAcquire.lowerBound)

        let datasetSource = try productionSource(
            "xabber/controllers/chats/chat/extension/ChatViewController+Dataset.swift"
        )
        XCTAssertGreaterThanOrEqual(
            offsets(of: "receipt: .skeleton", in: datasetSource).count,
            2,
            "Both async and synchronous skeleton applies must use the traced transaction"
        )
        let controllerSource = try productionSource(
            "xabber/controllers/chats/chat/ChatViewController.swift"
        )
        let transaction = try sourceSlice(
            controllerSource,
            from: "internal func performChatOpenPerformancePresentationTransaction(",
            to: "private func hasCommittedChatOpenPerformanceReceipt("
        )
        let logicalSkeletonCommit = try XCTUnwrap(
            transaction.range(of: "if receipt == .skeleton")
        )
        let skeletonReceipt = try XCTUnwrap(
            transaction.range(
                of: ".recordPresentationReceipt(",
                range: logicalSkeletonCommit.lowerBound..<transaction.endIndex
            )
        )
        let transactionCommit = try XCTUnwrap(
            transaction.range(
                of: "CATransaction.commit()",
                range: skeletonReceipt.upperBound..<transaction.endIndex
            )
        )
        XCTAssertLessThan(logicalSkeletonCommit.lowerBound, skeletonReceipt.lowerBound)
        XCTAssertLessThan(skeletonReceipt.lowerBound, transactionCommit.lowerBound)
    }

    func testProductionTraceContextCallsitesCoverInitialQueryMapPresentationAndLinkedPageApply() throws {
        let timelineSource = try productionSource(
            "xabber/controllers/chats/chat/ChatTimelineSession.swift"
        )
        XCTAssertTrue(timelineSource.contains(
        ...
        for selector in [
            ...
            "self.chatOpenPerformanceTraceLifecycle.beginPresenting(",
            "receipt: receipt",
            "registerInteractiveHistoryPagePerformanceTrace(",
            ...
            XCTAssertTrue(datasetSource.contains(selector), "Missing production selector: \(selector)")
        }
        XCTAssertGreaterThanOrEqual(
            offsets(
                of: "cancelInteractiveHistoryPagePerformanceTrace(",
                in: datasetSource
            ).count,
            5,
            "every remote arm rejection must tombstone its registered paging context"
        )

        let highPrioritySource = try productionSource(
            "xabber/controllers/chats/chat/rx/ChatViewController+HighPrioritySubscribtions.swift"
        )
        ...
        XCTAssertTrue(searchSource.contains(
            "let executor = self.performanceFixtureArchiveTransportExecutor"
        ))
    }

    func testInteractiveHistoryPageLoadContextInitializerRetainsEveryField() throws {
        let traceContext = try XCTUnwrap(ChatOpenPerformanceTraceContext(
            traceID: 101,
            generation: 202,
            kindCode: ChatOpenPerformanceTraceKind.paging.rawValue,
            purposeCode: ChatOpenPerformanceTracePurpose.explicitTargetRoute.rawValue
        ))
        let requestedWindow = ChatDatasetWindow(minIndex: 20, maxIndex: 60)
        let context = ChatInteractiveHistoryPageLoadContext(
            queryId: "query-with-trace",
            performanceTraceOwner: "owner@example.org",
            performanceTraceContext: traceContext,
            generation: 7,
            direction: .newer,
            chatPrimaryKey: "chat-primary",
            persistedCursorId: "persisted-cursor",
            persistedFullArchiveLoaded: true,
            requestedCursorId: "requested-cursor",
            requestedWindow: requestedWindow,
            preLoadObserverCount: 31,
            preLoadOldestArchivedId: "oldest-before-load",
            preLoadNewestArchivedId: "newest-before-load",
            preLoadFullArchiveLoaded: true,
            preLoadNewerLiveEdgeReached: false,
            remoteFetchStarted: true,
            isArchiveEndVerificationProbe: true,
            canMutateOlderArchiveEnd: true,
            expectedWindowMaxIndex: 60,
            coverageUpdateKind: .gapRepairNewer(cursorArchiveId: "requested-cursor"),
            ...
            resultCount: 13,
            persistedRowsForQuery: 14,
            visibleRowsForConversation: 15,
            didEnterObserverSettlePhase: true,
            didObservePostIdleTick: true,
            isArchivePagePersisted: true
        )

        XCTAssertEqual(context.queryId, "query-with-trace")
        XCTAssertEqual(context.performanceTraceOwner, "owner@example.org")
        XCTAssertEqual(context.performanceTraceContext, traceContext)
        XCTAssertEqual(context.generation, 7)
        XCTAssertEqual(context.direction, .newer)
        XCTAssertEqual(context.chatPrimaryKey, "chat-primary")
        XCTAssertEqual(context.persistedCursorId, "persisted-cursor")
        XCTAssertTrue(context.persistedFullArchiveLoaded)
        XCTAssertEqual(context.requestedCursorId, "requested-cursor")
        XCTAssertEqual(context.requestedWindow, requestedWindow)
        XCTAssertEqual(context.preLoadObserverCount, 31)
        XCTAssertEqual(context.preLoadOldestArchivedId, "oldest-before-load")
        XCTAssertEqual(context.preLoadNewestArchivedId, "newest-before-load")
        XCTAssertTrue(context.preLoadFullArchiveLoaded)
        XCTAssertFalse(context.preLoadNewerLiveEdgeReached)
        XCTAssertTrue(context.remoteFetchStarted)
        XCTAssertTrue(context.isArchiveEndVerificationProbe)
        XCTAssertTrue(context.canMutateOlderArchiveEnd)
        XCTAssertEqual(context.expectedWindowMaxIndex, 60)
        XCTAssertEqual(
            context.coverageUpdateKind,
            .gapRepairNewer(cursorArchiveId: "requested-cursor")
        )
        XCTAssertTrue(context.didReceiveEndPage)
        XCTAssertTrue(context.queryExhausted)
        XCTAssertEqual(context.persistedMessageCount, 12)
        XCTAssertEqual(context.resultFirst, "result-first")
        XCTAssertEqual(context.resultLast, "result-last")
        XCTAssertEqual(context.resultCount, 13)
        XCTAssertEqual(context.persistedRowsForQuery, 14)
        XCTAssertEqual(context.visibleRowsForConversation, 15)
        XCTAssertTrue(context.didEnterObserverSettlePhase)
        XCTAssertTrue(context.didObservePostIdleTick)
        XCTAssertTrue(context.isArchivePagePersisted)
    }

    func testInteractiveHistoryPageLoadContextInitializerPreservesLegacyDefaults() {
        let legacyContext = ChatInteractiveHistoryPageLoadContext(
            queryId: "legacy-query",
            direction: .older,
            chatPrimaryKey: "legacy-chat-primary",
            persistedCursorId: nil,
            persistedFullArchiveLoaded: false,
            requestedCursorId: nil,
            requestedWindow: .empty,
            ...
            expectedWindowMaxIndex: 0,
            coverageUpdateKind: .none
        )

        XCTAssertNil(legacyContext.performanceTraceOwner)
        XCTAssertNil(legacyContext.performanceTraceContext)
        XCTAssertEqual(legacyContext.generation, 0)
        XCTAssertFalse(legacyContext.didReceiveEndPage)
        XCTAssertFalse(legacyContext.queryExhausted)
        XCTAssertNil(legacyContext.persistedMessageCount)
        XCTAssertEqual(legacyContext.resultFirst, "")
        XCTAssertEqual(legacyContext.resultLast, "")
        XCTAssertEqual(legacyContext.resultCount, 0)
        XCTAssertEqual(legacyContext.persistedRowsForQuery, 0)
        XCTAssertEqual(legacyContext.visibleRowsForConversation, 0)
        XCTAssertFalse(legacyContext.didEnterObserverSettlePhase)
        XCTAssertFalse(legacyContext.didObservePostIdleTick)
        XCTAssertFalse(legacyContext.isArchivePagePersisted)
    }
