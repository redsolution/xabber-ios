...
## Task 26B-F1 — Исправить вертикальное optical alignment лупы

**Owner:** xabber-ui
**Secondary:** xabber-tests, xabber-lead
**Depends on:** Task 26A и отклонённая pre-commit runtime попытка Task 26B
**Commit:** fix(chat-search): align search magnifier optically

### Цель

Устранить подтверждённый runtime-дефект из screenshot `/Users/igor.boldin/Desktop/Simulator Screenshot - iPhone 16e - 2026-07-14 at 15.14.41.png`: `magnifyingglass` визуально расположен слишком низко внутри 44 pt leading control. Поднять только image content, не меняя hit-area, горизонтальную геометрию, размер символа, text-field frame или accessibility contract.
...
- этот plan/Execution journal;
- рабочие vault task/UI/tests notes, handoff и внешний execution ledger.
...
- Обе hosted-isolation переменные обязательны; использовать только exact iPhone 16e UDID.

### Tests-first

- Сначала изменить focused XCTest так, чтобы он требовал `top=0`, `bottom=8`, `leading=8`, `trailing=0` и vertical optical offset `-4 pt` внутри неизменного 44×44 frame.
- На production-коде с `top=8`, `bottom=0` зафиксировать ожидаемое red-падение: offset остаётся `+4 pt`, то есть изображение смещено вниз.
- После red менять production-константу; не ослаблять допуски и не заменять geometry assertion snapshot-only проверкой.

### Реализация

1. Перенести существующий 8 pt vertical content inset с верхней стороны на нижнюю; horizontal leading inset оставить 8 pt.
2. Сохранить 16 pt image point size, 44×44 button frame, manual framing и `translatesAutoresizingMaskIntoConstraints=true`.
3. Не менять glass surface, text field, cancel/clear/spinner, transitions, query/search behavior или XMPP/storage код.
4. После focused green повторить полный pre-task allowlist и mandatory cached simulator build без `clean`.
5. После отдельного source commit повторить Task 26A final union/build/install-over safety gate без нового Task 26A commit, затем перезапустить Task 26B с новой записью видео.

### Критерии принятия

- Конфигурация лупы имеет `top=0`, `bottom=8`, `leading=8`, `trailing=0`; vertical optical offset равен `-4 pt`.
- Дефектная версия доказанно падает focused test по top/bottom/offset, исправленная проходит.
- `submitButton.frame` остаётся 44×44 и символ не превышает 20×20; text field и остальные controls не двигаются.
- Полный post-task allowlist проходит без failures/skips.
- Cached build проходит на UDID `7C8F9347-C7DA-4EF2-9DA0-71A52E3B93AF`.
- Account/container/Realm не удаляются и не сбрасываются; runtime acceptance выполняется повтором Task 26A–26B после этого commit.

### Post-task verification

- TEST[ChatSearchTopChromeTests, ChatSearchAdaptiveLayoutTests, ChatSearchAccessibilityTests, ChatSearchLiveQASafetyPolicyTests, ChatSearchModeTransitionTests, SearchChatListKeyboardLayoutTests].
- Обязательная simulator build.
- Обновить journal/vault notes до `ready-to-commit`, выполнить `git diff --check`, создать один focused source commit и только затем записать его actual SHA во внешний ledger.

---

## Task 26B-F2 — Выполнить guarded calendar Done date-jump live test

**Owner:** xabber-tests
**Secondary:** xabber-ui, xabber-lead
**Depends on:** Task 26B-F1, повторённый Task 26A и отклонённая runtime-попытка Task 26B, доказавшая безусловный skip обязательного date-jump test
**Commit:** test(chat-search): execute guarded calendar date jump

### Цель

Устранить ложный pass в Task 26B: `testCalendarDateJumpForKnownResult` должен после успешного safety-gate реально открывать calendar, использовать уже выбранный civil day и нажимать Done. Проверка должна доказать, что search/list/calendar закрыты, safe date anchor выполнен и signed-in chat сохранён, без отправки/удаления сообщений и любой account/storage mutation.

### Файлы

- `xabberUITests/ChatSearchLiveSmokeTests.swift`;
- этот plan/Execution journal;
- рабочие vault task/tests/UI notes, handoff и внешний execution ledger.
...
- Hosted tests запускать только с `TEST_RUNNER_XABBER_DISABLE_ACCOUNT_AUTOCONNECT=1` и `TEST_RUNNER_XABBER_ISOLATED_STORAGE=1`.
- UI class без live opt-ins обязан skip-нуть оба test до создания `XCUIApplication`.

### Tests-first

- Заменить unconditional `XCTSkip` после `requireDateJumpAuthorization()` на вызов отсутствующего execution helper и зафиксировать ожидаемый compile-red `cannot find 'executeControlledCalendarDateJump' in scope`.
- После helper compile-green повторить no-opt-in class: оба test должны остаться skipped до app launch.
- Реальный date-jump запускать только с обоими live opt-ins, exact UDID, Andrew Nenakhov → Alexey Boldin и exact lowercase query `test`.

### Реализация

1. Создавать `XCUIApplication` только после успешной authorization и дожидаться signed-in shell; login/onboarding — failure, не повод вводить credentials.
2. Найти первый permitted dialog с непустой terminal result, проверить exact query и initial committed position `1 of N`.
3. Открыть calendar, проверить ровно один `isSelected` day с непустым accessibility identifier/label и enabled/hittable Done.
4. Нажать Done и дождаться исчезновения calendar, search input, result count и loader; проверить возврат в тот же signed-in chat без login/onboarding.
5. Приложить before/after screenshots и text evidence с dialog/query/initial position/selected-day identity; teardown ограничить process termination.
6. Не вызывать send/edit/delete/logout/login, uninstall, reset, container/Realm cleanup и не читать/менять credentials.
7. Уложиться в shared 180 s global live-QA budget, повторить hosted post allowlist и mandatory cached simulator build.
8. После отдельного source commit повторить Task 26A final union/build/install-over gate без нового Task 26A commit, затем полностью перезапустить Task 26B с новой video recording.

### Критерии принятия

- Compile-red подтверждает отсутствие execution helper; тот же exact class после implementation compile-ится.
- No-opt-in class исполняет два skip до app creation; live date-jump с обоими opt-ins исполняется без skip/failure.
- Live run использует Andrew Nenakhov или Alexey Boldin, exact query `test`, ровно один preselected day и enabled Done.
- Done закрывает весь search/list/calendar flow, а не только calendar; selected dialog и signed-in shell сохранены.
- Hosted post allowlist проходит без failures/skips; cached build проходит на exact iPhone 16e.
- Main Realm inode/size и account presence сохранены; destructive cleanup/account mutation не выполнялись.

### Post-task verification

- Compile-only TEST[ChatSearchLiveSmokeTests].
- No-opt-in TEST[ChatSearchLiveSmokeTests]: ровно 2 skipped, 0 failures, до app launch.
- Hosted TEST[ChatSearchLiveQASafetyPolicyTests] и exact 10-suite pre-task allowlist.
- Guarded TEST[ChatSearchLiveSmokeTests/testCalendarDateJumpForKnownResult] с обоими opt-ins: 1 passed, 0 skipped.
- Обязательная simulator build; exact account/container/Realm check; journal/vault update; `git diff --check`; один focused source commit; actual SHA во внешний ledger только после commit.
...
~~~bash
UDID='7C8F9347-C7DA-4EF2-9DA0-71A52E3B93AF'
VIDEO='/Users/igor.boldin/Downloads/Xabber_Search_Parity_2026-07-14_goal-qa-final.mp4'
...
| 26B | ready-to-commit | Hosted pre-task 10-suite allowlist 124/124 pass, 0 skipped; no-opt-in UI class 2/2 skipped before app creation; signed-in preflight preserved Andrew/Alexey and Realm inode `171969950` | Runtime integration evidence: guarded Calendar Done and full parity cases both executed and passed, 2/2, 0 skipped; 50.128 s + 176.493 s | Required post-task hosted allowlist 89/89 pass, 0 skipped | pass — corrected `tools/xcodebuild_cached.sh build` invocation, exact iPhone 16e `7C8F9347-C7DA-4EF2-9DA0-71A52E3B93AF`; the earlier duplicate-argument invocation exited before build and is not acceptance evidence | docs(chat-search): record visual parity evidence | Fresh strict-center video SHA `6566e4bd…`, 1170×2532, 240.843333 s; both live cases used Andrew Nenakhov and exact `test`. PTS comparison accepts local top/bottom geometry at 0 pt delta, strict magnifier midpoint `0 pt`, result/list/navigation/calendar/Done behavior and shared animation durations. Data-only query-case/count/content deltas are recorded. Final wrapper `EACCA696-16C1-4CDB-9B1D-C7FAD40F2B54`, Realm inode/size preserved, signed-in Chats still shows Andrew/Alexey; no destructive operation ran. |
| 26C | ready-to-commit | Required closure pre-task allowlist 109/109 pass, 0 skipped; Task 26A and accepted Task 26B evidence verified | Documentation-only: 38 prior ledger rows resolved to 38 unique source commits; one external Task 18 SHA transcription corrected to reachable `1db9df7a…`; no production/test change | Required closure post-task allowlist 74/74 pass, 0 skipped | pass — `tools/xcodebuild_cached.sh build`, exact iPhone 16e `7C8F9347-C7DA-4EF2-9DA0-71A52E3B93AF` | docs(chat-search): close Telegram parity goal | Durable feature/provider/state/safety contract added and QA closure recorded. Final ordinary launch preserved wrapper `EACCA696-16C1-4CDB-9B1D-C7FAD40F2B54`, Realm inode `171969950`, size `19,087,360`, signed-in Chats and Andrew/Alexey. External ledger has 38 actual SHA plus a Task 26C placeholder; actual Task 26C SHA is intentionally written only after this source commit. |

## 14. Durable project links

- Durable feature contract: [In-chat search](../features/in-chat-search.md)
- Final verification: [Telegram-style in-chat search verification](../qa/telegram-style-in-chat-search-verification.md)
- Vault task after Task 26C closure: /Users/igor.boldin/projects/xabber/xabber/projects/xabber/tasks/done/xab-telegram-style-in-chat-search-goal-plan.md
