...
- Idle, loading, empty and error states show only a 40×40 pt circular calendar surface. Its glyph is centered in that circle and fits within 24×24 pt while the real hit and accessibility target remains at least 44×44 pt. The first positive result expands the surface to a 144 pt capsule and shows `N messages`; only successful timeline positioning changes it to `current of total`. `No messages` is never rendered or exposed to accessibility.
