...
## Post-goal refinement — loading-safe live terminal observation

- The first unobstructed refinement retry was invalid as product evidence: while `chat_search_loading` was visible, the live harness attempted to read `value`, `label` and `identifier` from a missing result-counter element. XCUI produced six snapshot failures per test and the run ended before the configured 45-second terminal timeout, so it did not prove that the MAM provider was stuck.
- The harness now treats loading as non-terminal and routes every result-counter property read through one guarded observation. Loading is checked before existence, after existence and after the single value read. A missing element is checked only for existence; committed-position parsing, result-presence gating and legacy-empty fallback reuse the same observation.
- A precommit `N messages` counter blocks empty-state inference. Counterless empty requires two consecutive non-loading snapshots with the search input still present and no result controls, so a transition gap is not terminal and a fast empty response remains detectable even when its spinner is missed. Error, paging and navigation probes use the same loading boundary.
- Pre-task safety allowlist passed 23/23 in `Test-Debug-2026.07.16_15-08-00-+0500.xcresult`. Initial compile-red `Test-Debug-2026.07.16_15-15-30-+0500.xcresult` proved the guard was absent. Review then exposed an unguarded paging path and decorative-submit empty fallback: strengthened compile-red is `Test-Debug-2026.07.16_15-25-57-+0500.xcresult`, and stable-empty behavior red is retained in `Test-Debug-2026.07.16_15-28-13-+0500.xcresult` plus `Test-Debug-2026.07.16_15-29-32-+0500.xcresult`.
- Final focused policy tests passed 4/4 in `Test-Debug-2026.07.16_15-30-52-+0500.xcresult`; the complete safety postflight passed 27/27 in `Test-Debug-2026.07.16_15-31-24-+0500.xcresult`. Both hosted isolation flags and only explicit selectors were used.
- The cached production build for iPhone 16e `7C8F9347-C7DA-4EF2-9DA0-71A52E3B93AF` ended with `** BUILD SUCCEEDED **`. A fresh install-over/account-preservation check and the guarded Andrew Nenakhov/Alexey Boldin flow with exact query `test` remain required before this refinement task can close.
